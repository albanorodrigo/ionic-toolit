...
.controller('SoporteCtrl', function ($rootScope, $scope, $q, $ionicPopup, $ionicModal, $timeout, 
  BroadcastService, AuthService, SoporteService, FormularioCierreService, FormularioCierreData, UsuarioService, 
  APP_EVENTS, APP_MODULOS, USER_ROLES, APP_CONFIG) {
...
//  };
  ...
  $scope.checkDocumentoCierreObligatorio = function () {
    return $scope.getModuloClienteConfigValue(APP_MODULOS.soportes.id, APP_MODULOS.soportes.config.documentoCierreObligatorio);
  };
  ...
  $scope.getSoportePrioridadClass = function (soporte) {
    if (soporte && soporte.prioridad) {
      if (soporte.prioridad.prioridad > 0) {
        return "ion-arrow-up-c " + $scope.getSoportePrioridadColorClass(soporte);
      } else if (soporte.prioridad.prioridad < 0) {
        return "ion-arrow-down-c " + $scope.getSoportePrioridadColorClass(soporte);
      }
    }
    return "";
  };

  $scope.getSoportePrioridadColorClass = function (soporte) {
    if (soporte && soporte.prioridad) {
      switch (soporte.prioridad.prioridad) {
        case  2: return "priority-very-high"; break;
        case  1: return "priority-high"; break;
        case  0: return "priority-normal"; break;
        case -1: return "priority-low"; break;
        case -2: return "priority-very-low"; break;
      }
    }
    return "";
  };
  ...
  /**
   * devuelve el atraso del soporte haciendo la diferencia entre la fechaHoraPlaneado/tsCreación
   * y el día actual, el formato del objecto que devuelve es
   * { diff: NNNNNNN, dias: NN, texto: "hace 4 dias" }
   * @param {type} soporte
   * @returns {Number|String}
   */
  $scope.getSoporteAtraso = function(soporte) {
    var tsSoporte = (soporte.fechaHoraPlaneado ? soporte.fechaHoraPlaneado : soporte.tsCreacion);
    var inputFormat = APP_CONFIG.defaultAppTsFormat;
    var ret = {diff:0, dias:0, texto:'hoy'};
    if (tsSoporte && inputFormat) {
      var md = moment(tsSoporte, inputFormat).startOf('day');
      var hoy = moment().startOf('day');
      ret.diff = moment().diff(md);
      ret.dias = hoy.diff(md,'days'); //Math.floor(moment.duration(diff).asDays());
      // si es hoy, el texto es "hoy"
      if (ret.dias === 0) {
        ret.texto = "hoy"; // ("+moment(tsSoporte, inputFormat).fromNow()+")";
      } else {
        // si no es hoy...
        if (ret.dias > 0) {
          // es atrasado, el texto es "XX día(s)" (después la interfaz le agrega "de atraso" si es el caso)
          if (ret.dias === 1) {
            ret.texto = "1 día";
          } else {
            ret.texto = ret.dias +" días";
          }
          //ret.texto = moment(tsSoporte, inputFormat).fromNow(true); //+" de atraso";
        } else {
          // es en el futuro
          if (ret.dias === -1) {
            ret.texto = "mañana";
          } else {
            ret.texto = "en " + Math.abs(ret.dias) +" días";
          }
          //ret.texto = moment(tsSoporte, inputFormat).fromNow();
        }
      }
      //if (moment(tsSoporte, inputFormat).fromNow() < 0) 
      //return moment(tsSoporte, inputFormat).fromNow().duration().asHour();
      //moment(moment().diff(moment(tsSoporte, inputFormat))).format('d h')
      //return moment(moment().diff(moment(tsSoporte, inputFormat)));//.format('HH');
    }
    return ret;
  };

  /**
   * devuelve la clase para colorear el header dependiendo de los dias de atraso
   * @param {type} dias
   * @returns {String}
   */
  $scope.getSoporteAtrasoColorClass = function (dias) {
    if (dias) {
      if (dias > 1) {
        return "atraso-mucho";
      } else if (dias === 1) {
        ...
        return "atraso-futuro";
      }
    }
    return "atraso-no";
  };
  
  $scope.itemHeight = function(soporte) {
    return soporte.isSeparator ? "50px" : "100px";
  };
